...
def cost_state(x_result, x_target, Q):
    return 0.5 * np.append(x_result - x_target, 1).T.dot(Q.dot(np.append(x_result - x_target, 1)))


def cost_control(u_result, u_target, R):
    return 0.5 * (u_result - u_target).T.dot(R.dot(u_result - u_target))


def cost_final(x_result, x_target, Qfinal):
    return 0.5 * np.append(x_result - x_target, 1).T.dot(
        Qfinal.dot(np.append(x_result - x_target, 1))
    )
